...
class TestPayslipBase(common.TransactionCase):
    ...
    def setUp(self):
        ...
        self.jurisdiction_id = self.data_model.get_object_reference(
            cr, uid, 'l10n_ca_hr_payroll', 'jurisdiction_federal')[1]
        ...
        self.public_activity_id = self.activity_model.search(
            cr, uid, [('code', '=', 'PUBLIC')], context=context)[0]

        self.sl_activity_id = self.activity_model.search(
            cr, uid, [('code', '=', 'SL')], context=context)[0]

        self.unpaid_activity_id = self.activity_model.search(cr, uid, [
            ('type', '=', 'leave'),
            ('leave_id.paid_leave', '=', False)
        ], context=context)[0]
...
class TestHrPayslip(TestPayslipBase):
    ...
    def setUp(self):
        ...
        for line in [
            ...
            ('2014-11-17', '2014-11-17', 11, self.job_activity_id, 15, 90),
            ('2014-11-18', '2014-11-25', 13, self.vac_activity_id, 10, 110),
            ('2014-11-30', '2014-11-30', 23, self.comp_activity_id, 13, 150),
        ]:
    ...
    def test_onchange_activity_id_leave(self):
        """
        Test onchange_activity_id with a leave activity
        """
        cr, uid, context = self.cr, self.uid, self.context

        res = self.worked_days_model.onchange_activity_id(
            cr, uid, [], '2014-11-17', '2014-11-31',
            self.vac_activity_id, self.contract_id,
            context=context
        )

        self.assertEqual(res['value']['hourly_rate'], 40)

    def test_onchange_activity_id_job(self):
        """
        Test onchange_activity_id with a job position
        """
        cr, uid, context = self.cr, self.uid, self.context

        res = self.worked_days_model.onchange_activity_id(
            cr, uid, [],
            '2014-11-17', '2014-11-31',
            self.job_2_activity_id, self.contract_id,
            context=context
        )

        self.assertEqual(res['value']['hourly_rate'], 40)

    def test_onchange_activity_id_job_not_found(self):
        """
        Test onchange_activity_id with a job position, when
        the hourly rate is not found
        """
        cr, uid, context = self.cr, self.uid, self.context
        ...
        self.assertEqual(res['value']['hourly_rate'], 0)

    def test_onchange_activity_id_rate_from_wage(self):
        """
        Test onchange_activity_id with a job position, when
        employee is paid by wage
        """
        cr, uid, context = self.cr, self.uid, self.context
        ...
        # 52000 / (80 hours per pay * 26 pays per year) = 25
        self.assertEqual(res['value']['hourly_rate'], 25)
