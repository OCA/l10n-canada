...
class test_worked_days_from_timesheet(common.TransactionCase):
    ...
    def test_import_worked_days_wage(self):
        ...
        hourly_rate = 50000.0 / (52 * 40)

        self.assertEqual(
            round(sub_totals[self.job_1_activity_id], 2),
            round((8 + 13 + 14) * hourly_rate, 2))

        self.assertEqual(
            round(sub_totals[self.job_2_activity_id], 2),
            round((9 + 15) * hourly_rate, 2))

        # The employee takes 11 hours of vacations
        # his main job's hourly rate is taken (35 / hour)
        self.assertEqual(
            round(sub_totals[self.vac_activity_id], 2),
            round(11 * hourly_rate, 2))
